...
exports.addUser = function (username, callback) {
  var options = {m: '', s: '/usr/sbin/nologin'};
  return new Promise(function(resolve, reject) {
    if (!isString(username)) {
      Object.assign(options, username);
    ...
    Object.keys(options).forEach(option=>{
      cmd += ' -' + option + ((options[option] !== '') ? ' ' + options[option] : '');
    });
    cmd += ' ' + username;
...
exports.isUser = function(username, callback) {
  return exports.getUserInfo(username).then(info=>{
    if (callback) {
      callback(null, true);
    }
    return true;
